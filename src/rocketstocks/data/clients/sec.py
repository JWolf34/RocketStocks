...
class SEC():
    def __init__(self, get_cik_fn):
        self.headers = {"User-Agent":"johnmwolf34@gmail.com"}
        self.MAX_CALLS = 10
        self.MAX_CALLS = 1
        self._get_cik = get_cik_fn
    ...
    def get_submissions_data(self, ticker):
        ...
        submissions_json = requests.get(f"https://data.sec.gov/submissions/CIK{self._get_cik(ticker)}.json", headers=self.headers).json()
    ...
    def get_link_to_filing(self, ticker, filing):
        return f"https://sec.gov/Archives/edgar/data/{self._get_cik(ticker).lstrip('0')}/{filing['accessionNumber'].replace('-','')}/{filing['primaryDocument']}"
    ...
    def get_accounts_payable(self, ticker):
        ...
        try:
            json = requests.get(f"https://data.sec.gov/api/xbrl/companyconcept/CIK{self._get_cik(ticker)}/us-gaap/AccountsPayableCurrent.json", headers=self.headers).json()
    ...
    def get_company_facts(self, ticker):
        ...
        try:
            json = requests.get(f"https://data.sec.gov/api/xbrl/companyfacts/CIK{self._get_cik(ticker)}.json", headers=self.headers).json()
            return json
        except requests.exceptions.JSONDecodeError as e:
