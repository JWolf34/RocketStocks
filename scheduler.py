...
def scheduler():
    ...
    sched.add_job(sd.daily_download_data, 'cron', name = 'Download data and generate indictor data for all tickers in the masterlist', timezone = timezone, hour = 23, minute = 22, replace_existing=True)

    # Generate indicators on downloaded data and update the data file
    sched.add_job(an.generate_indicators, 'cron', name = 'Fenerate indicator data for masterlist tickers', timezone = timezone, hour = 23, minute = 29, replace_existing=True)
