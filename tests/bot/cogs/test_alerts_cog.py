...
def _make_simple_alerts_cog():
    """Create an Alerts cog with minimal setup for slash-command tests."""
    bot = _make_bot()
    bot.get_channel_for_guild = AsyncMock(return_value=MagicMock())
    sd = MagicMock(name="StockData")
    sd.alert_roles.get_all_for_guild = AsyncMock(return_value={})
    sd.db = MagicMock()
    sd.db.execute = AsyncMock(return_value=None)
    sd.price_history.fetch_daily_price_history_batch = AsyncMock(return_value={})
    with (
...
class TestResolveSinceDt:
    ...
    def test_market_open_today(self):
        from rocketstocks.bot.cogs.alerts import _resolve_since_dt
...
class TestSendSubscriptionSelect:
    ...
    def _make_interaction(self, guild_id=123, user_role_ids=None):
        interaction = MagicMock(spec=_discord.Interaction)
        interaction.guild_id = guild_id
        roles = []
        for rid in (user_role_ids or []):
            role = MagicMock(spec=_discord.Role)
            role.id = rid
            roles.append(role)
        interaction.user = MagicMock(spec=_discord.Member)
    ...
    async def test_send_subscription_select_sends_ephemeral_message(self):
        ...
        with patch("rocketstocks.bot.cogs.alerts.AlertSubscriptionSelect"), \
             patch("rocketstocks.bot.cogs.alerts.AlertSubscriptionView"):
...
class TestAlertSummaryCommand:
    ...
    async def test_defaults_to_last_close_when_no_args(self):
        ...
        with (
            ...
            patch("rocketstocks.bot.cogs.alerts.send_report", new_callable=AsyncMock, return_value=mock_message),
            patch("rocketstocks.bot.cogs.alerts._resolve_since_dt", return_value=(
    ...
    async def test_private_by_default(self):
        ...
        with (
            patch.object(cog, 'build_alert_summary', new_callable=AsyncMock, return_value=mock_content),
            patch("rocketstocks.bot.cogs.alerts.send_report", new_callable=AsyncMock, return_value=mock_message) as mock_send,
            patch("rocketstocks.bot.cogs.alerts._resolve_since_dt", return_value=(
                datetime.datetime(2026, 3, 7, 21, 0), "since last close (Mar 07)"
            )),
        ):
